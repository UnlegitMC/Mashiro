...
class ViewListener : UltralightViewListener {
    ...
    override fun onChangeURL(url: String) {
        Mashiro.ultralightManager.logger.warn("View url has changed: $url")
    }
    ...
    override fun onChangeCursor(cursor: UltralightCursor) {
        // TODO: Cursor Adaptor
//        Mashiro.ultralightManager.cursorAdapter.notifyCursorUpdated(cursor)
    }
     ...
     */
    override fun onAddConsoleMessage(source: MessageSource, level: MessageLevel, message: String, lineNumber: Long, columnNumber: Long, sourceId: String) {
        Mashiro.ultralightManager.logger.info("View message: [${source.name}/${level.name}] $sourceId:$lineNumber:$columnNumber: $message")
    }
     ...
     * Ultralight
     */
    override fun onCreateChildView(openerUrl: String, targetUrl: String, isPopup: Boolean, popupRect: IntRect): UltralightView? {
